...
#include <string.h>
struct _neo_engine_scope_t {
  neo_engine_scope_t parent;
  neo_list_t children;
  neo_list_t variables;
  neo_map_t named_variables;
  neo_engine_handle_t root;
  uint32_t ref;
};

static bool neo_engine_scope_check_alive(neo_allocator_t allocator,
...
static void neo_engine_scope_gc(neo_allocator_t allocator,
                                neo_engine_scope_t self) {
  neo_list_t children = neo_engine_handle_get_children(self->root);
  neo_list_t workflow = neo_create_list(allocator, NULL);
  neo_list_t cache = neo_create_list(allocator, NULL);
  ...
  while (neo_list_get_size(children)) {
    ...
    neo_engine_handle_t child = neo_list_node_get(it);
    neo_engine_handle_remove_parent(child, self->root);
  ...
  while (neo_list_get_size(workflow)) {
    ...
    neo_list_push(cache, item);
    if (!neo_engine_scope_check_alive(allocator, item)) {
      children = neo_engine_handle_get_children(item);
      while (neo_list_get_size(children)) {
        neo_list_node_t it = neo_list_get_first(children);
        neo_engine_handle_t child = neo_list_node_get(it);
        neo_list_push(workflow, child);
        neo_engine_handle_remove_parent(child, item);
      }
...
static void neo_engine_scope_dispose(neo_allocator_t allocator,
                                     neo_engine_scope_t self) {
  if (self->parent) {
  ...
  while (neo_list_get_size(self->children)) {
    ...
    neo_engine_scope_t child = neo_list_node_get(it);
  ...
  neo_engine_scope_gc(allocator, self);
  neo_allocator_free(allocator, self->root);
}

neo_engine_scope_t neo_create_js_scope(neo_allocator_t allocator,
                                       neo_engine_scope_t parent) {
  neo_engine_scope_t scope = neo_allocator_alloc(
      allocator, sizeof(struct _neo_engine_scope_t), neo_engine_scope_dispose);
  if (parent) {
    scope->parent = parent;
...
uint32_t neo_engine_scope_ref(neo_engine_scope_t self) { return self->ref; }
...
neo_engine_scope_t neo_engine_scope_set_parent(neo_engine_scope_t self,
                                               neo_engine_scope_t parent) {
  neo_engine_scope_t current = self->parent;
  self->parent = parent;
  return current;
}
...
neo_engine_scope_create_variable(neo_allocator_t allocator,
                                 neo_engine_scope_t self,
                                 neo_engine_handle_t handle) {
  neo_engine_variable_t variable = neo_create_js_variable(allocator, handle);
  neo_engine_handle_add_parent(handle, self->root);
  neo_list_push(self->variables, variable);
  return variable;
}