...
neo_js_variable_t neo_js_array_from(neo_js_context_t ctx,
  ...
  if (neo_js_context_has_field(ctx, array_like, iterator)) {
    ...
    if (neo_js_variable_get_type(generator)->kind == NEO_TYPE_ERROR) {
      return generator;
    }
    ...
    for (uint32_t idx = 0; idx < num->number; idx++) {
      ...
      if (map_fn) {
        ...
        if (neo_js_variable_get_type(item)->kind == NEO_TYPE_ERROR) {
          return item;
        }
      }
...
neo_js_variable_t neo_js_array_is_array(neo_js_context_t ctx,
  ...
  if (neo_js_variable_get_type(item)->kind >= NEO_TYPE_OBJECT) {
...
neo_js_variable_t neo_js_array_constructor(neo_js_context_t ctx,
                                           ...
                                           uint32_t argc,
                                           neo_js_variable_t *argv) {
  if (argc == 1 && neo_js_variable_get_type(argv[0])->kind == NEO_TYPE_NUMBER) {
    uint32_t length = 0;
...
neo_js_variable_t neo_js_array_concat(neo_js_context_t ctx,
                                      neo_js_variable_t self, uint32_t argc,
                                      neo_js_variable_t *argv) {
  if (neo_js_variable_get_type(self)->kind == NEO_TYPE_UNDEFINED ||
      neo_js_variable_get_type(self)->kind == NEO_TYPE_NULL) {
    return neo_js_context_create_simple_error(
        ctx, NEO_ERROR_TYPE,
  ...
  for (size_t count = 0; count < argc + 1; count++) {
    ...
    if (neo_js_variable_get_type(item)->kind < NEO_TYPE_OBJECT && count == 0) {
      item = neo_js_context_to_object(ctx, item);
      if (neo_js_variable_get_type(item)->kind == NEO_TYPE_ERROR) {
        return item;
      }
    }
    if (neo_js_variable_get_type(item)->kind == NEO_TYPE_ARRAY) {
      neo_js_variable_t vlength = neo_js_context_get_field(
    ...
    } else if (neo_js_variable_get_type(item)->kind < NEO_TYPE_OBJECT) {
      neo_js_variable_t error = neo_js_context_set_field(
...
neo_js_variable_t neo_js_array_copy_within(neo_js_context_t ctx,
  ...
  if (neo_js_variable_get_type(self)->kind < NEO_TYPE_OBJECT) {
    ...
    if (neo_js_variable_get_type(self)->kind == NEO_TYPE_ERROR) {
      return self;
    }
  }
  neo_js_variable_t vlength = neo_js_context_get_field(
...
neo_js_variable_t neo_js_array_flat(neo_js_context_t ctx,
  ...
  for (double idx = 0; idx < length; idx += 1) {
    ...
    neo_js_variable_t item = neo_js_context_get_field(ctx, self, key);
    NEO_JS_TRY_AND_THROW(item);
    if (neo_js_variable_get_type(item)->kind == NEO_TYPE_ARRAY && depth >= 1) {
      neo_js_variable_t error =
...
neo_js_variable_t neo_js_array_join(neo_js_context_t ctx,
  ...
  for (double idx = 0; idx < length; idx += 1) {
    ...
    if (neo_js_variable_get_type(item)->kind != NEO_TYPE_UNDEFINED &&
        neo_js_variable_get_type(item)->kind != NEO_TYPE_NULL) {
...
neo_js_variable_t neo_js_array_reduce(neo_js_context_t ctx,
  ...
  if (length == 0 && argc < 2) {
    return neo_js_context_create_simple_error(
        ctx, NEO_ERROR_TYPE, L"Reduce of empty array with no initial value");
  }
  if (length == 0) {
    return argv[1];
  }
...
neo_js_array_quick_sort_part(neo_js_context_t ctx, neo_list_t list,
  ...
  while (it != end) {
    ...
    if (flag) {
      if (neo_js_variable_get_type(flag)->kind == NEO_TYPE_UNDEFINED) {
        if (!item ||
            neo_js_variable_get_type(item)->kind == NEO_TYPE_UNDEFINED) {
