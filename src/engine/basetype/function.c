...
static const wchar_t *neo_engine_function_typeof() { return L"function"; }

static neo_engine_variable_t
neo_engine_function_to_string(neo_engine_context_t ctx,
                              neo_engine_variable_t self) {
  neo_engine_function_t function =
      neo_engine_value_to_function(neo_engine_variable_get_value(self));
  if (function->callable.source) {
    return neo_engine_context_create_string(ctx, function->callable.source);
  }
  if (function->callable.name) {
    const wchar_t *name =
        neo_engine_value_to_string(
            neo_engine_handle_get_value(function->callable.name))
            ->string;
    neo_engine_runtime_t runtime = neo_engine_context_get_runtime(ctx);
    neo_allocator_t allocator = neo_engine_runtime_get_allocator(runtime);
    wchar_t *source = neo_allocator_alloc(allocator, wcslen(name) + 32, NULL);
    swprintf(source, wcslen(name) + 32, L"function %ls(){ [native code] }",
             name);
    neo_engine_variable_t string =
        neo_engine_context_create_string(ctx, L"function (){ [native code] }");
    neo_allocator_free(allocator, source);
    return string;
  }
  return neo_engine_context_create_string(ctx, L"function (){ [native code] }");
}
static neo_engine_variable_t
neo_engine_function_get_field(neo_engine_context_t ctx,
                              neo_engine_variable_t self,
  ...
  if (neo_engine_variable_get_type(field)->kind == NEO_TYPE_STRING) {
    ...
    if (wcscmp(string->string, L"name") == 0) {
      neo_engine_function_t function =
          neo_engine_value_to_function(neo_engine_variable_get_value(self));
      if (!function->callable.name) {
        return neo_engine_context_create_string(ctx, L"");
      } else {
        return neo_engine_context_create_variable(ctx, function->callable.name);
      }
    }
  }
...
static neo_engine_variable_t neo_engine_function_set_field(
...
static neo_engine_variable_t
neo_engine_function_del_field(neo_engine_context_t ctx,
                              neo_engine_variable_t self,
                              neo_engine_variable_t field) {
  neo_engine_type_t otype = neo_get_js_object_type();
  if (neo_engine_variable_get_type(field)->kind == NEO_TYPE_STRING) {
    ...
    if (wcscmp(string->string, L"name") == 0) {
      ...
      if (function->callable.name) {
        ...
        neo_engine_handle_add_parent(function->callable.name, root);
        neo_engine_handle_remove_parent(function->callable.name,
                                        neo_engine_variable_get_handle(self));
...
neo_engine_type_t neo_get_js_function_type() {
  ...
  neo_engine_type_t otype = neo_get_js_object_type();
  type.typeof_fn = neo_engine_function_typeof;
  ...
  type.get_field_fn = neo_engine_function_get_field;
  type.set_field_fn = neo_engine_function_set_field;
  type.del_field_fn = neo_engine_function_del_field;
  type.is_equal_fn = otype->is_equal_fn;
  type.copy_fn = otype->copy_fn;
  return &type;
}

static void neo_engine_function_dispose(neo_allocator_t allocator,
                                        neo_engine_function_t self) {
...
neo_engine_function_t neo_create_js_function(neo_allocator_t allocator,
  ...
  neo_engine_function_t function =
      neo_allocator_alloc(allocator, sizeof(struct _neo_engine_function_t),
                          neo_engine_function_dispose);
...
neo_engine_function_t neo_engine_value_to_function(neo_engine_value_t value) {
  if (value->type->kind >= NEO_TYPE_FUNCTION) {
    return (neo_engine_function_t)value;
  }
  return NULL;
}