...
static neo_js_variable_t neo_js_async_cfunction_set_field(
    neo_js_context_t ctx, neo_js_variable_t self, neo_js_variable_t field,
...
neo_js_async_cfunction_copy_fn(neo_js_context_t ctx, neo_js_variable_t self,
  ...
  for (neo_hash_map_node_t it = neo_hash_map_get_first(func->callable.closure);
    ...
    neo_js_handle_add_parent(handle, htarget);
  }
  return target;
}
...
neo_js_type_t neo_get_js_async_cfunction_type() {
  ...
  type.get_field_fn = neo_js_async_cfunction_get_field;
