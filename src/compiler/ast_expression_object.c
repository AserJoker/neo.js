...
#include "neojs/compiler/ast_object_accessor.h"
#include "neojs/compiler/ast_object_method.h"
#include "neojs/compiler/ast_object_property.h"
...
static void neo_ast_expression_object_write(neo_allocator_t allocator,
                                            ...
                                            neo_ast_expression_object_t self) {
  neo_js_program_add_code(allocator, ctx->program, NEO_ASM_PUSH_OBJECT);
  for (neo_list_node_t it = neo_list_get_first(self->items);
       it != neo_list_get_tail(self->items); it = neo_list_node_next(it)) {
...
neo_serialize_ast_expression_object(neo_allocator_t allocator,
  ...
  neo_any_set(
      variable, "type",
      neo_create_any_string(allocator, "NEO_NODE_TYPE_EXPRESSION_OBJECT"));
...
neo_ast_node_t neo_ast_read_expression_object(neo_allocator_t allocator,
  ...
  if (*current.offset != '}') {
    for (;;) {
      ...
      if (!item) {
        item = neo_ast_read_object_accessor(allocator, file, &current);
      }
      if (!item) {
        item = neo_ast_read_object_method(allocator, file, &current);
      }
      if (!item) {
        item = neo_ast_read_expression_spread(allocator, file, &current);
      }
      if (!item) {
        item = neo_ast_read_object_property(allocator, file, &current);
      }
      if (!item) {
