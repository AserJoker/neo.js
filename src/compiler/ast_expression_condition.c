...
neo_serialize_ast_expression_condition(neo_allocator_t allocator,
  ...
  neo_any_set(
      variable, "type",
      neo_create_any_string(allocator, "NEO_NODE_TYPE_EXPRESSION_CONDITION"));
...
neo_ast_node_t neo_ast_read_expression_condition(neo_allocator_t allocator,
  ...
  node->condition = neo_ast_read_expression_3(allocator, file, &current);
  if (!node->condition) {
    goto onerror;
  }
  ...
  node->consequent = neo_ast_read_expression_2(allocator, file, &current);
  if (!node->consequent) {
  ...
  node->alternate = neo_ast_read_expression_2(allocator, file, &current);
  if (!node->alternate) {
    error = neo_create_error_node(
        allocator, "Invalid or unexpected token \n  at _.compile (%s:%d:%d)",
        file, current.line, current.column);
