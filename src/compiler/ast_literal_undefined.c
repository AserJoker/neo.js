#include "neo.js/compiler/ast_literal_undefined.h"
...
static void
neo_ast_literal_undefined_dispose(neo_allocator_t allocator,
                                  neo_ast_literal_undefined_t node) {
  neo_allocator_free(allocator, node->node.scope);
}

static neo_any_t
neo_serialize_ast_literal_undefined(neo_allocator_t allocator,
                                    neo_ast_literal_undefined_t node) {
  neo_any_t variable = neo_create_any_dict(allocator, NULL, NULL);
  neo_any_set(
      variable, "type",
      neo_create_any_string(allocator, "NEO_NODE_TYPE_LITERAL_UNDEFINED"));
...
neo_create_ast_literal_undefined(neo_allocator_t allocator) {
  ...
  node->node.serialize =
      (neo_serialize_fn_t)neo_serialize_ast_literal_undefined;
  node->node.resolve_closure = neo_ast_node_resolve_closure;
  node->node.write = (neo_write_fn_t)neo_ast_literal_undefined_write;
  return node;
}
...
neo_ast_node_t neo_ast_read_literal_undefined(neo_allocator_t allocator,
  ...
  neo_ast_literal_undefined_t node = NULL;
  ...
  if (!token || !neo_location_is(token->location, "undefined")) {
    goto onerror;
  }
  neo_allocator_free(allocator, token);
  node = neo_create_ast_literal_undefined(allocator);
