...
#include "core/any.h"
#include <stdio.h>
static void neo_ast_statement_for_in_dispose(neo_allocator_t allocator,
...
static void neo_ast_statement_for_in_write(neo_allocator_t allocator,
  ...
  neo_js_program_add_code(allocator, ctx->program, NEO_ASM_KEYS);
...
neo_serialize_ast_statement_for_in(neo_allocator_t allocator,
  ...
  neo_any_set(
      variable, "type",
      neo_create_any_string(allocator, "NEO_NODE_TYPE_STATEMENT_FOR_IN"));
...
static neo_ast_statement_for_in_t
neo_create_ast_statement_for_in(neo_allocator_t allocator) {
  neo_ast_statement_for_in_t node =
      neo_allocator_alloc(allocator, sizeof(struct _neo_ast_statement_for_in_t),
                          neo_ast_statement_for_in_dispose);
  ...
  node->node.write = (neo_write_fn_t)neo_ast_statement_for_in_write;
  node->left = NULL;
  node->right = NULL;
  node->body = NULL;
...
neo_ast_node_t neo_ast_read_statement_for_in(neo_allocator_t allocator,
  ...
  neo_ast_statement_for_in_t node = neo_create_ast_statement_for_in(allocator);
