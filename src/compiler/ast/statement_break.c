...
neo_serialize_ast_statement_break(neo_allocator_t allocator,
  ...
  neo_any_set(
      variable, "type",
      neo_create_any_string(allocator, "NEO_NODE_TYPE_STATEMENT_BREAK"));
...
neo_ast_node_t neo_ast_read_statement_break(neo_allocator_t allocator,
  ...
  if (cur.line == line) {
    node->label = neo_ast_read_identifier(allocator, file, &cur);
    if (node->label) {
      NEO_CHECK_NODE(node->label, error, onerror);
      current = cur;
      line = cur.line;
      error = neo_skip_all(allocator, file, &cur);
      if (error) {
