...
int main(int argc, char *argv[]) {
  ...
  neo_js_variable_t string = neo_js_context_create_string(ctx, L"1.5e2");
  neo_js_variable_t num = neo_js_context_to_number(ctx, string);
  neo_js_variable_t output = neo_js_context_to_string(ctx, num);
  const wchar_t *result = neo_js_string_get_value(
      neo_js_value_to_string(neo_js_variable_get_value(output)));
  printf("%ls\n", result);
